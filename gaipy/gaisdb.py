...
def Select(db, pattern={}, filter_args={},ret_col=[] ,mode='', page_cnt=10, page=1, order_by='', order='decreasing'):
    if page_cnt < 1 :
        ...
        if type(ret_col) == list and len(ret_col) != 0:
            for i in range(len(ret_col)):
                ret_col[i] = __parse_to_gaisrec(ret_col[i])
            cmd += '&select=%s' % ','.join(ret_col)
        response = requests.get(cmd)
        gaisdb_res = response.json()
